...
const SearchBar = ({ showSearch, setShowSearch }) => {
  ...
  const inputStyles = `${showSearch ? 'w-[62.5vw] mx-auto transition-all duration-500 ml-2 pl-4 py-2' : 'max-sm:hidden'} md:w-[36vw] lg:w-[42vw] md:py-[7px] lg:py-[7px] border border-gray-300 rounded-l-full py-1 pl-3 md:pl-6 transition-all focus:outline-0 duration-500 ${isInputFocused ? 'max-sm:w-[75.5vw] max-sm:mx-auto max-sm:focus:outline-0 md:pl-[3.2rem] border border-blue-700' : ''} ${isMenuOpen ? 'md:w-[29vw] md:ml-20 lg:ml-0' : ''}`;

  const searchButtonStyles = `${showSearch ? 'px-3 text-lg' : 'max-sm:border-none max-sm:text-2xl max-sm:ml-[7.5rem]'} border border-gray-300 hover:bg-gray-200 rounded-r-full md:px-3 flex justify-center items-center md:bg-gray-100 ${isInputFocused ? 'border border-blue-700' : ''}`;
