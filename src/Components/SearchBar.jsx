...
const SearchBar = ({ showSearch, setShowSearch }) => {
  ...
  const inputStyles = `${showSearch ? 'w-[61vw] mx-auto transition-all duration-500 ml-2 pl-4 py-2' : 'max-sm:hidden'} md:w-[36vw] lg:w-[42vw] md:py-[7px] lg:py-[7px] border border-gray-300 md:rounded-l-full max-sm:rounded-l-xl py-1 pl-3 md:pl-6 transition-all focus:outline-0 duration-500 ${isInputFocused ? 'max-sm:w-[75.5vw] max-sm:mx-auto max-sm:focus:outline-0 md:pl-[3.2rem] md:border md:border-blue-500 max-sm:border max-sm:border-blue-500 ' : ''} ${isMenuOpen ? 'md:w-[30vw] md:ml-20 lg:ml-0' : ''}`;

  const searchButtonStyles = `${showSearch ? 'max-sm:px-3 max-sm:py-[10px] max-sm:rounded-r-xl' : 'max-sm:border-none max-sm:text-2xl max-sm:px-auto max-sm:ml-32'} text-xl px-[2px] py-[9px] border border-gray-300 hover:bg-gray-200 rounded-r-full md:px-6 flex justify-center items-center md:bg-gray-100 ${isInputFocused ? 'md:border md:border-blue-500 max-sm:border max-sm:border-blue-500 ' : ''}`;
...

          {
            isInputFocused && (
              <div className={`absolute z-30 max-sm:hidden flex items-center`}>
                <div className={`${isMenuOpen ? 'md:ml-[6.4rem]' : 'lg:ml-[1.4rem] md:ml-[1.4rem]'}`}>
                  <IoSearchOutline className='text-xl' />
                </div>
                <div className={`${isMenuOpen ? 'md:ml-[10rem]' : 'lg:ml-[29rem] md:ml-[12.5rem]'}`}>
